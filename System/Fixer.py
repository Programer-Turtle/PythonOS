...
def InstallErrorManagerBackup():
    BackUpCode = """#Do not run independently
...
"""
    with open(os.path.join("System", "errormanager.py"), "w") as Backup:
        Backup.write(BackUpCode)
        return

def InstallSystemActionsBackup():
    BackUpCode = """import time
import os
...
def DeleteProgramUI():
    ...
    for counter, programs in enumerate(ProgramList, start=1):
        print(f"{counter}.{programs}")
    print("What program would you like to delete? IMPORTANT program can be recovered!")
    while True:
